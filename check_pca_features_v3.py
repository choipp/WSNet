...
def process_model(cfg_path, checkpoint_path, image_tensor, use_original_hook=False):
    ...
    fusion_output = custom_forward(model, image_input)
...
def feature_visualize(models, input_image):
    ...
    for model in models:
        ...
        #plt.tight_layout()
        plt.savefig(os.path.join(BASE_DIR,"results_3",f"feat_{model['label']}.png"))
        plt.close()

        ipython_display(os.path.join(BASE_DIR,"results_3",f"feat_{model['label']}.png"))
        ...
        fig, axs = plt.subplots(1, 10, figsize=(24, 12))
        for i in range(10):
            ax = axs[i]
            ax.imshow(similar_images[0][i], cmap='gray', vmin=-1, vmax=1)
            ax.axis('off')

        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        plt.tight_layout()
        plt.savefig(os.path.join(BASE_DIR,"results_3",f"similar_images_{model['label']}.png"), dpi=600)
        plt.close()        

        ipython_display(os.path.join(BASE_DIR,"results_3",f"similar_images_{model['label']}.png"))
...
def plot_histogram(models, input_image, n_components):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR,"results","histogram_multiple_models_with_stages.png"))
    plt.close()

    #ipython_display(Image.open('histogram_multiple_models_with_stages.png'))

plot_histogram(models, image_tensor, 5)
...
def plot_histogram_2(models, input_image):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR,"results_3","histogram_combined.png"))
    plt.show()
    plt.close()
